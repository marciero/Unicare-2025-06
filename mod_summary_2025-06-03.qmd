...
Overall
```{r}
patients_events %>% summarize(mean_count = mean(inpatient_count))
```

By cohort
```{r}
patients_events %>% summarize(.by = cohort, mean_count = mean(inpatient_count))
```
...
By cohort
```{r}
patients_events %>% summarize(.by = cohort, prop_zero_admit = mean(inpatient_count == 0))
```
...
Overall
```{r}
patients_events %>% summarize(mean_at_least_one_admit = mean(inpatient_count > 0))
```
