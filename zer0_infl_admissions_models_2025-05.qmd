...
$$\textrm{counts}_i \sim NB(\mu_i, \theta)$$ 
$$\log \mu_i = \beta_0 + \beta_1 \textrm{cohort}_i$$
...
```{r}
setwd("/Users/marciero/Dropbox/Projects/Senscio/2025-Senscio")

## data from boot2.R
load("data/boot_samples_admissions.RData")


boot_df <- bind_rows(boot_samples_nb_admit, .id = "replicate")

boot_stats4 <- boot_df %>% group_by(cohort,inpatient_count) %>%
...
boot_stats4 %>% full_join(obs_freqs_cohort_inpatient) %>%
    ...
    labs(x = "Number of admissions", y =  "probability/proportion",
         title = "Probability estimates and observed proportions",
         fill = NULL,
         color = NULL,
         shape = NULL) 
```
...
```{r, eval = FALSE}
## Omit as before for rendering tables
legend_df <- tibble(
    x = 11 , y = 0,
...
mod_nb_predict_admit(patients_events) %>% 
    ...
    geom_point(aes(y = expected),
                    color = clrs5[1])  +
    geom_line(aes(y = expected), color = clrs5[1], alpha = 0.5) +
    geom_point(aes(y = frequency), color = clrs4[4], shape = 15, size = 2) +
    geom_line(aes(y = frequency), color = clrs4[4], alpha = 0.7) +
    ...
    scale_color_manual(values = c("Expected" = clrs5[1], "Observed" = clrs4[4])) +
    scale_shape_manual(values = c("Expected" = 16, "Observed" = 15)) +
    ...
    labs(x = "Number of admissions", y =  "frequency",
         title = "Expected and observed frequencies",
         fill = NULL,
         color = NULL,
         shape = NULL) 
...
mod_zinb_predict_admit(patients_events) %>% 
    ...
    geom_point(aes(y = expected),
                    color = clrs5[1])  +
    geom_line(aes(y = expected), color = clrs5[1], alpha = 0.5) +
    geom_point(aes(y = frequency), color = clrs4[4], shape = 15, size = 2, alpha = 0.5) +
    geom_line(aes(y = frequency), color = clrs4[4], alpha = 0.7) +
...
count_coefs <- tidy(mod_zinb1, component = "count") %>%
    mutate(component = "count") %>% select(term, component, estimate, p.value) %>%  mutate(p.value = round(p.value, 4))
zero_coefs <- tidy(mod_zinb1, component = "zero") %>%
    mutate(component = "zero") %>%  select(term, component, estimate, p.value) %>%  mutate(p.value = round(p.value, 4))

dfzinb1 <- bind_rows(count_coefs, zero_coefs) 
dfzinb1 <- dfzinb1 %>% mutate(model = "ZINB1") 

count_coefs <- tidy(mod_zip2, component = "count") %>%
    mutate(component = "count") %>% select(term, component, estimate, p.value) %>%  mutate(p.value = round(p.value, 4))
zero_coefs <- tidy(mod_zip2, component = "zero") %>%
    mutate(component = "zero") %>%  select(term, component, estimate, p.value) %>%  mutate(p.value = round(p.value, 4))

dfzip2 <- bind_rows(count_coefs, zero_coefs) 
dfzip2 <- dfzip2 %>% mutate(model = "ZIP2") 

count_coefs <- tidy(mod_zinb2, component = "count") %>%
    mutate(component = "count") %>% select(term, component, estimate, p.value) %>%  mutate(p.value = round(p.value, 4))
zero_coefs <- tidy(mod_zinb2, component = "zero") %>%
    mutate(component = "zero") %>%  select(term, component, estimate, p.value) %>%  mutate(p.value = round(p.value, 4))
...
df_zipzinb <- bind_rows(dfzip1, dfzinb1, dfzip2, dfzinb2)
...
\log \mu_i & = \beta_0 + \beta_1 \textrm{cohort}_i \\
\log \frac{\pi_i}{1 - \pi_i} & = \gamma_0 + \gamma_1 \textrm{condition\_count}_i\\
\end{aligned}
$$
...
```{r}
dftemp <- fixef(zif_bayes) %>% as.data.frame() %>% 
...
mcmc_hist(zif_bayes, pars = c("b_cohortibis", "b_chf", "b_atrial_fibrillation"),
...
::: {.column width="50%"}
```{r}
mcmc_intervals(zif_bayes, pars = vars(starts_with("b_"))) +
...
```{r}
y_rep <- posterior_predict(zif_bayes)

y_obs <- zif_bayes$data$inpatient_count
